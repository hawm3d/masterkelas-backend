...
/**
 * Master Encryption from Laravel Encryption
 *
 * @since      1.0.0
 * @package    MasterKelas
...
class Encrypter {
  ...
  public function __construct($key, $cipher = 'aes-128-cbc') {
    ...
    if (!static::supported($key, $cipher)) {
      ...
      throw new MasterException("Unsupported cipher or incorrect key length. Supported ciphers are: {$ciphers}.");
    }
  ...
  public function encrypt($value, $serialize = true) {
    ...
    if ($value === false) {
      throw new MasterException('encryption.failed');
    }
  ...
  public function decrypt($payload, $unserialize = true) {
    ...
    if ($decrypted === false) {
      throw new MasterException('decryption.failed');
    }
  ...
  protected function getJsonPayload($payload) {
    ...
    if (!$this->validPayload($payload)) {
      throw new MasterException('decryption.invalid.payload');
    }

    if (!self::$supportedCiphers[strtolower($this->cipher)]['aead'] && !$this->validMac($payload)) {
      throw new MasterException('decryption.invalid.mac');
    }
  ...
  protected function ensureTagIsValid($tag) {
    ...
    if (!self::$supportedCiphers[strtolower($this->cipher)]['aead'] && is_string($tag)) {
      throw new MasterException('Unable to use tag because the cipher algorithm does not support AEAD.');
    }
  }
