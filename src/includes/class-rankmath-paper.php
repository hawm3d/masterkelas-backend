...
class RankMathPaper {
  ...
  public static function get($query, $args = []) {
    ...
    self::$instance = new RankMathPaper();
    self::$query = $query;
  ...
  private function setup($args = []) {
    foreach ($this->get_papers($args) as $class_name => $is_valid) {
    ...
    $this->paper->set_object(self::$query);
   ...
   * @return array
   */
  private function get_papers($args = []) {
    $defaults = [
      'Search'    => is_search(),
   ...
   */
  public static function reset($query = null) {
    self::$instance = null;
