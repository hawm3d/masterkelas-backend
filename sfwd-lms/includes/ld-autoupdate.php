...
if ( ! class_exists( 'nss_plugin_updater_sfwd_lms' ) ) {
	...
	class nss_plugin_updater_sfwd_lms {
		...
		public function check_update( $transient ) {
			...
			// If the update_path is not set then abort.
			...
			if ( ( ! empty( $this->update_path ) ) && ( $this->time_to_recheck_license() ) ) {
				...
				if ( ! is_wp_error( $request ) || wp_remote_retrieve_response_code( $request ) === 200 ) {
					...
					if ( '1' !== $request_body ) {
						 ...
						 */
						update_option( 'nss_plugin_remote_license_' . $this->slug, array( 'value' => 'active' ), LEARNDASH_PLUGIN_LICENSE_OPTIONS_AUTOLOAD );
					}
